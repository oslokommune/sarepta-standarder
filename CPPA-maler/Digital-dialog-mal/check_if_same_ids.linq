...
void Main()
{
	...
	File.WriteAllLines(Path.Combine(directory, "ids_in_receiver.txt"), GetIDsUsed(receiverXml, true).OrderBy(x => x));
	File.WriteAllLines(Path.Combine(directory, "ids_in_sender.txt"), GetIDsUsed(senderXml, true).OrderBy(x => x));
}
